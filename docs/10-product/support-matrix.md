...
| PHP-FPM | Ubuntu 24.04 apt PHP 8.3 FPM | Observe + standard `php.ini` `G2 VM_PASS` |
...
| Managed config | adapter allowlisted resource | Nginx active + PHP 8.3 FPM `php.ini` profiles `G2 VM_PASS`; others after gate |
...
- PHP 8.3 FPM은 version·unit·활성 extension·masked config path를 관찰합니다. 표준 `/etc/php/8.3/fpm/php.ini`만 valid save·문법 오류·정확 원복·service continuity VM gate를 통과해 `SUPPORTED + VM_PASS + G2`입니다.
