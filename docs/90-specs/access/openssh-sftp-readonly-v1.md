...
upload, create, delete, move, chmod, text save는 이 spec 밖이며 UI와 API에서 차단합니다. same-directory temporary file, exclusive create, fsync, digest precondition, atomic rename, interruption cleanup과 별도 G1 승인이 VM fault evidence를 통과한 뒤에만 승격합니다.
