...
2026-07-22 local evidence는 Nginx operation의 모든 non-terminal durable stage 재시작 판정과 fixed runner의 allowlist·environment clear·bounded streams·timeout을 검증했습니다. Ubuntu 24.04 source-level runner test는 자손 process group의 pipe 회수까지 통과했습니다. 실제 installed daemon을 각 stage에서 강제 종료하는 전체 package matrix는 아직 후속 `p2-vm` 증거입니다.
