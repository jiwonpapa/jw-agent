...
최상위 위험은 credential stuffing, 공개 HTTP input으로 agentd를 침해한 뒤 root authd·opsd 경계를 공격하는 연쇄 경로, PAM FFI 결함, P2 path·crash recovery 오류, OpenSSH manual session 탈취, Certbot 외부효과와 package 공급망입니다. P2 Nginx site-state·active managed-config와 Certbot one-shot renewal dry-run은 전용 Ubuntu 24.04 VM에서 package·PAM·TLS·성공·실패·비밀 비노출·원장 훼손 `VM_PASS`를 획득했습니다. 다만 private-LAN test CA와 unsigned local package 증거이므로 공인 배포·운영 안전으로 과장하지 않으며 terminal·SFTP·Certbot issuance/attach는 각 VM 증거 전까지 구현 완료를 주장하지 않습니다.
...
| TM-013 | stolen authenticated session | terminal/SFTP ticket issuance allowed | open manual session, run command, alter writable file | service/data compromise within user/sudo rights | OpenSSH account, host data | non-root, PAM reauth, purpose-bound short ticket, no transcript persistence contract | credential broker/dependency/runtime proof 없음 | deny UID0, require step-up, bind and revoke sessions, VM sudo/root/protected-root negatives | session start/end, ticket replay and policy denial | medium | high | critical |
...
- P1 public HTTPS template, recovery, REST, authd/PAM, opsd capability, DB, package, xtask와 P2 active design surfaces를 포함했습니다.
- 모든 trust boundary를 하나 이상의 threat와 연결했습니다.
- 현재 P1 local·Ubuntu VM control, unsigned/test-CA 한계, P2 Accepted contract와 미구현 증거를 분리했습니다.
