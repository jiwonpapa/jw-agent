...
최상위 위험은 credential stuffing, 공개 HTTP input으로 agentd를 침해한 뒤 root authd·opsd 경계를 공격하는 연쇄 경로, PAM FFI 결함, session·CSRF, package 공급망입니다. P1 runtime은 전용 Ubuntu 24.04 VM에서 PAM·systemd·Nginx·TLS·`.deb` 범위의 `VM_PASS`를 획득했지만 private-LAN test CA와 unsigned local package 증거이므로 공인 배포·운영 안전으로 과장하지 않습니다. P1 opsd는 읽기 전용 capability만 응답하며 일반 서비스 쓰기는 없습니다.
...
- 구현된 agentd, one-shot authd, ffi-pam, read-only opsd
...
- arbitrary shell·terminal·SFTP·file manager
- P2 operation ledger·snapshot·service mutation과 release artifact
...
- P2 이후 Certbot guided issuance를 지원할지 여부와 typed operation 범위
...
- opsd: P1 root networkless stateless read-only capability responder
- Ubuntu services: PAM/NSS, systemd, Nginx and observed services
- Local stores: P1 agentd session/settings SQLite; opsd ledger/snapshot은 P2 전에는 없음
...
- agentd → opsd: P1 read-only capability frame over separate versioned UDS; password never crosses.
- opsd → Ubuntu/Nginx: P1에는 변경 flow가 없습니다. P2 진입 후에만 fixed argv와 typed filesystem primitive를 별도 모델링합니다.
...
- Browser input cannot contain arbitrary path, argv or shell text.
...
| Nginx site operation (P2 planned) | approved plan | opsd → filesystem/systemd | P1 runtime에는 없음 | `docs/90-specs/operations/nginx-site-state-set-v1.md` |
...
| TM-008 | future P2 attacker | P2 write primitive is introduced without new gate | exploit symlink/path/TOCTOU | arbitrary root path change | config, host | P1 has no service mutation or operation ledger; scope gate in `AGENTS.md` | P2 primitive intentionally absent | require accepted P2 entry and no-follow/dirfd VM proof before adding operation | scope/gate drift failure | low | high | low |
| TM-009 | future P2 fault actor | durable operation state is introduced | kill, reboot or disk full during apply | repeated apply or failed recovery | future ledger, snapshot, config | P1 opsd is stateless/read-only; lifecycle spec only | implementation intentionally absent | add persist-before-effect, quota, read-back and kill-stage VM matrix only in P2 | future recovery-required and disk alerts | low | high | low |
...
가장 우선할 실제 검토 경로는 `crates/jw-agentd/src/api.rs`, `crates/jw-agentd/src/session.rs`, `crates/jw-authd/src/main.rs`, `crates/ffi-pam/src/linux.rs`, `crates/jw-opsd/src/main.rs`, `packaging/systemd/`, `packaging/nginx/`입니다. P2 filesystem primitive와 release signer가 생기면 별도 focus path로 추가합니다.

## Quality check

- P1 public HTTPS template, recovery, REST, authd/PAM, opsd capability, DB, package, xtask entry points와 P2 비활성 경계를 포함했습니다.
- 모든 trust boundary를 하나 이상의 threat와 연결했습니다.
- 현재 P1 local·Ubuntu VM control, unsigned/test-CA 한계, 미래 P2 mitigation을 분리했습니다.
