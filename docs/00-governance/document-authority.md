...
각 사실은 한 문서만 소유합니다. 다른 문서는 요약을 복사하지 않고 링크합니다.
구현·지원·증거·제외 상태는 [Capability Registry](capability-registry.md)가 소유하고
사람이 읽는 상태표를 생성합니다. 실행 가능한 GateId와 lane 구성은 `xtask`가 계속
단독 소유하며 문서에 개수를 복사하지 않습니다.
