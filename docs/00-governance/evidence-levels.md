...
현재 문서·Rust·계약·웹 unit/build와 mock browser 범위는 `LOCAL_PASS`를, 전용 Ubuntu 24.04 VM의 PAM·systemd·Nginx·TLS·`.deb` 설치·site-state·active managed-config·Certbot 갱신과 발급 실패 안전성은 `VM_PASS`를 획득했습니다. 이 증거는 private-LAN `.test` 호스트와 unsigned local package에 한정되며, 공인 DNS·공인 CA 발급 성공·서명 release 또는 운영 안전을 주장하지 않습니다.
