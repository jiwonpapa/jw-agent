...
- `p2-local` 19개, `p2-browser` 8개 gate와 Playwright 22개 scenario가 PASS했습니다.
- `p2-vm` 19개 gate에서 Nginx fault matrix, forensic lockdown, Certbot one-shot boundary·sanitized inventory·G1 renewal과 guided issuance의 CA 실패 안전성을 검증했습니다.
- Ubuntu 24.04 VM에 `jw-agent_0.2.0~p2.6_amd64.deb`를 설치했고 SHA-256은 `a906624b81c91217bd234718893d9c1052103e80e87587f631bb4d357ea639b8`입니다.
...
현재 one-shot `jw-certd` root-only UDS, fixed argv, email argv 비노출과 digest-only output은 `VM_PASS`입니다. 표준 lineage의 SAN·만료·fingerprint·timer 조회와 `certbot.certificate.renew_test/v1`도 실제 VM에서 정상/비정상 timer 양쪽을 검증했습니다. `certbot.certificate.issue/v1`은 DNS exact-match, 80/443 listener, 보호된 Nginx webroot, staging-first, PAM과 두 개의 외부효과 동의를 요구하며, 계정 이메일은 root 0600 임시 proposal 뒤 삭제됩니다. private-LAN `.test` VM에서는 실제 CA 실패가 `REJECTED`되고 inventory·Nginx가 보존되는 경로까지 `VM_PASS`입니다. 공인 CA 발급 성공과 Nginx attach·local rollback은 아직 `UNVERIFIED`입니다.
