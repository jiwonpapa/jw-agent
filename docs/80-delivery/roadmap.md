...
- `p2-local` 19개, `p2-browser` 8개 gate와 Playwright 19개 scenario가 PASS했습니다.
- `p2-vm` 16개 gate에서 site-state와 managed-config fault matrix, forensic lockdown, Certbot one-shot privilege boundary를 검증했습니다.
- Ubuntu 24.04 VM에 `jw-agent_0.2.0~p2.3_amd64.deb`를 설치했고 SHA-256은 `51352a295956992e7832344524bdbb7e6c342461839f46d4605a025307eedef7`입니다.
...
현재 one-shot `jw-certd` root-only UDS, fixed argv, email argv 비노출, digest-only output, Ubuntu Certbot renewal dry-run과 worker cleanup은 `VM_PASS`입니다. 발급·certificate read-back·Nginx attach·rollback API/UI는 아직 `UNSUPPORTED`이며 완료로 표시하지 않습니다.
