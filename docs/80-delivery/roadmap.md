...
- `p2-local` 21개, `p2-browser` 8개 gate와 Playwright 30개 scenario가 PASS했습니다.
- `p2-vm` 22개 gate에서 Nginx fault matrix, forensic lockdown, Certbot lifecycle, non-root OpenSSH terminal과 home-scoped read-only SFTP를 검증했습니다.
- Ubuntu 24.04 VM에 `jw-agent_0.2.0~p2.9_amd64.deb`를 설치했고 SHA-256은 `3ceff4b35814a543c7368b2a7210036dfed14a204cf14e76fc9c5e2a935d7e16`입니다.
...
- G1 bounded atomic upload와 별도 승인·충돌·부분전송 계약
- G1 text editing과 Monaco 도입 여부를 build graph ADR 뒤 결정
...
터미널은 Ubuntu 24.04 package VM에서 non-root PAM/OpenSSH 로그인, 명령 I/O, 40×100 resize, replay·wrong-origin 차단, logout revoke, metadata-only audit와 process/FIFO cleanup을 `VM_PASS + G1`로 검증했습니다. SFTP G0는 같은 loopback OpenSSH 경계에서 홈 list/stat/text/download, traversal·absolute path·외부 symlink·size·cross-session·wrong-origin·close/logout 차단, path digest audit와 process/FIFO cleanup을 별도 `VM_PASS`로 검증했습니다. VM의 password 인증은 `Match LocalAddress 127.0.0.1`에만 허용해 LAN SSH 정책을 넓히지 않았습니다. upload·edit·delete·move·chmod는 구현하지 않았습니다.
