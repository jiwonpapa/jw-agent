...
- `p2-local` 19개, `p2-browser` 8개 gate와 Playwright 21개 scenario가 PASS했습니다.
- `p2-vm` 18개 gate에서 Nginx fault matrix, forensic lockdown, Certbot one-shot boundary·sanitized inventory·G1 renewal operation을 검증했습니다.
- Ubuntu 24.04 VM에 `jw-agent_0.2.0~p2.5_amd64.deb`를 설치했고 SHA-256은 `294364da081405ba36746817816fb8049551a4905303d662f2e11777a7d21944`입니다.
...
현재 one-shot `jw-certd` root-only UDS, fixed argv, email argv 비노출과 digest-only output은 `VM_PASS`입니다. 표준 lineage의 SAN·만료·fingerprint·timer 조회와 모바일 G0 UI, 계획·G1 비원복 경고·PAM 재인증·private snapshot·실행·timer read-back을 잇는 `certbot.certificate.renew_test/v1`도 실제 VM에서 정상/비정상 timer 양쪽을 검증했습니다. 발급·Nginx attach·local rollback API/UI는 아직 `UNSUPPORTED`입니다.
