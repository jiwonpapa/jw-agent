...
- `p2-local` 19개, `p2-browser` 8개 gate와 Playwright 20개 scenario가 PASS했습니다.
- `p2-vm` 17개 gate에서 Nginx fault matrix, forensic lockdown, Certbot one-shot boundary와 sanitized inventory를 검증했습니다.
- Ubuntu 24.04 VM에 `jw-agent_0.2.0~p2.4_amd64.deb`를 설치했고 SHA-256은 `47d8e57718c15c8933fc10e7b00bed5b5111791378e8e1dbbff91039103de9db`입니다.
...
현재 one-shot `jw-certd` root-only UDS, fixed argv, email argv 비노출, digest-only output, Ubuntu Certbot renewal dry-run과 worker cleanup은 `VM_PASS`입니다. 표준 lineage의 SAN·만료·fingerprint·timer 조회와 모바일 G0 UI도 `VM_PASS`이며 실제 공개 화면 console error 0을 확인했습니다. 발급·Nginx attach·rollback API/UI는 아직 `UNSUPPORTED`입니다.
