...
- `p2-local` 22개, `p2-browser` 8개 gate와 Playwright 38개 scenario가 PASS했습니다.
- `p2-vm` 24개 gate에서 서비스 인벤토리, Nginx·PHP-FPM fault matrix, forensic lockdown, Certbot lifecycle, non-root OpenSSH terminal과 home-scoped SFTP G0/G1을 검증했습니다.
- Ubuntu 24.04 VM에 `jw-agent_0.2.0~p2.15_amd64.deb`를 설치했고 SHA-256은 `3684319fa35e2be9af9f7ec475923d41cc041285a67d0f12831ba07dee06b51e`입니다.
...
웹 terminal·SFTP session owner는 route component에서 authenticated app shell로 이동했습니다. 메뉴 이동은 연결을 닫지 않으며 명시적 종료·logout·서버 idle/max lifetime만 종료합니다. 같은 ticket/token을 재사용하는 browser gate와 p2.15 실제 OpenSSH endpoint VM gate가 통과했습니다.
