...
P2 전체 milestone은 아직 완료가 아닙니다. 모든 durable stage의 package-daemon 강제 종료
matrix는 후속 adapter 전에 계속 보강합니다. command timeout·output cap은
managed-config validator의 실제 package/VM fault로 검증했습니다.
...
- legacy Nginx site resource는 24 KiB, service-tree와 PHP-FPM resource는 128 KiB이며 exact managed-config plan path·ops frame만 256 KiB입니다. root:root regular file과 typed reload·validate-only만 허용합니다.
...
- 15분 관리 모드 진입용 PAM+TOTP direct context와 고위험 exact-plan용 5분 single-use claim
- browser-memory QR와 mobile-friendly 등록 ceremony

VM에서 `risky_operations` 정책 활성화, PAM+TOTP 관리 모드 진입, 그 안의 Nginx typed no-op 승인, 동일 time-step code 재사용 403, 복구 코드 reset, session revoke, 암호화 key 권한과 평문 secret 비보존을 검증했습니다. Linux PAM·SSH MFA 변경, 공용 bypass와 console recovery 자동화는 제공하지 않습니다.
