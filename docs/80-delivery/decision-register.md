...
- `p2-local` 19개, `p2-browser` 8개, Playwright 21개, `p2-vm` 18개 gate가 PASS했습니다.
- VM package는 `jw-agent_0.2.0~p2.5_amd64.deb`, SHA-256 `294364da081405ba36746817816fb8049551a4905303d662f2e11777a7d21944`입니다.
...
- 현재 `SUPPORTED + VM_PASS + G2` write 표면은 `nginx.site_state.set/v1`과 active Nginx profile의 `service.config_file.set/v1`이며, `certbot.certificate.renew_test/v1`은 `SUPPORTED + VM_PASS + G1`입니다. P2D는 gate 전까지 지원 완료로 표시하지 않습니다.
- `jw-certd` one-shot network privilege boundary, sanitized certificate inventory와 PAM 승인 renewal dry-run은 `VM_PASS`이며 Certbot 발급·attach API는 아직 노출하지 않습니다.
