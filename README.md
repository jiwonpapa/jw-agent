...
현재 단계는 `P2 Safe local operations`입니다. durable safety kernel, Nginx G2 작업, Certbot one-shot 경계·인증서 조회·G1 갱신 dry-run과 guided 발급의 안전한 CA 실패 경로를 Ubuntu VM에서 검증했습니다. 공인 CA 발급 성공·Nginx attach와 terminal·SFTP를 포함한 P2 전체 완료는 아직 주장하지 않습니다.
...
- sanitized 인증서 inventory, 갱신 dry-run, DNS·webroot preflight 기반 Certbot 발급 계획·PAM 승인
