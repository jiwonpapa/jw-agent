...
import { operationErrorCopy, problemLabel } from "./certificate-errors";
...
export function CertificatesScreen() {
  ...
  async function createRenewPlan(
    ...
    if (!administrativeConfirmed && session?.administrativeAccess !== "administrative") {
      requestAccess(() => void createRenewPlan(data, true));
      return;
    }
    if (
  ...
  }, administrativeConfirmed = false): Promise<void> {
    if (!administrativeConfirmed && session?.administrativeAccess !== "administrative") {
      requestAccess(() => void createIssuePlan(input, true));
      return;
    }
  ...
  async function createAttachPlan(
    certificate: CertificateSummaryView,
    administrativeConfirmed = false,
  ): Promise<void> {
    if (!administrativeConfirmed && session?.administrativeAccess !== "administrative") {
      requestAccess(() => void createAttachPlan(certificate, true));
      return;
    }
    const data = inventory.data;
    const publicHost = accessSettings.data?.publicHost;
